...
class TestSpecAugment:
    ...
    def test_output_shape_preserved(self, dummy_mel):
        aug = SpecAugment()
        out = aug(dummy_mel)
        assert out.shape == dummy_mel.shape

    def test_creates_zeros(self):
        aug = SpecAugment(freq_mask_param=30, time_mask_param=30,
    ...
    def test_freq_mask_only_frequency_axis(self):
        aug = SpecAugment(freq_mask_param=20, time_mask_param=0,
                          n_freq_masks=2, n_time_masks=0)
        x   = torch.ones(1, 1, 128, 128)
        out = aug(x)
        # Los ceros deben estar en filas (frecuencia), no en columnas (tiempo)
        zero_rows = (out[0, 0].sum(dim=1) == 0).sum().item()
        assert zero_rows > 0

    def test_time_mask_only_time_axis(self):
        aug = SpecAugment(freq_mask_param=0, time_mask_param=20,
                          n_freq_masks=0, n_time_masks=2)
        x   = torch.ones(1, 1, 128, 128)
        out = aug(x)
        zero_cols = (out[0, 0].sum(dim=0) == 0).sum().item()
        assert zero_cols > 0
...
class TestPANNSModel:
    ...
    def test_output_shape(self, panns_model, dummy_mel):
        with torch.no_grad():
            out = panns_model(dummy_mel)
        assert out.shape == (BATCH, N_CLASSES)

    def test_embedding_shape(self, panns_model, dummy_mel):
        with torch.no_grad():
            emb = panns_model(dummy_mel, return_embedding=True)
        assert emb.shape == (BATCH, 2048)

    def test_get_embeddings_l2_normalized(self, panns_model, dummy_mel):
        with torch.no_grad():
            emb = panns_model.get_embeddings(dummy_mel)
        norms = emb.norm(dim=-1)
        assert torch.allclose(norms, torch.ones_like(norms), atol=1e-5)

    def test_predict_proba_sums_to_one(self, panns_model, dummy_mel):
        with torch.no_grad():
            probs = panns_model.predict_proba(dummy_mel)
        sums = probs.sum(dim=-1)
        assert torch.allclose(sums, torch.ones_like(sums), atol=1e-5)
    ...
    def test_gradient_head_only_when_frozen(self, panns_model, dummy_mel, dummy_labels):
        panns_model.freeze_backbone()
        logits = panns_model(dummy_mel)
        loss   = nn.CrossEntropyLoss()(logits, dummy_labels)
        loss.backward()
...
class TestIntegration:
    ...
    def test_specaugment_then_forward(self, cnn_model, dummy_mel, dummy_labels):
        """Pipeline: SpecAugment → forward → loss → backward."""
        aug    = SpecAugment(freq_mask_param=10, time_mask_param=10)
        x_aug  = aug(dummy_mel)
        logits = cnn_model(x_aug)
        loss   = nn.CrossEntropyLoss()(logits, dummy_labels)
        loss.backward()
    ...
    def test_mixup_then_forward(self, cnn_model, dummy_mel, dummy_labels):
        ...
        loss          = lam * nn.CrossEntropyLoss()(logits, ya) + \
                        (1 - lam) * nn.CrossEntropyLoss()(logits, yb)
        loss.backward()
        assert not torch.isnan(loss)
    ...
    def test_both_augmentations_pipeline(self, cnn_model, dummy_mel, dummy_labels):
        ...
        x     = aug(dummy_mel)
        x, ya, yb, lam = mixup(x, dummy_labels)
        logits = cnn_model(x)
        loss   = lam * nn.CrossEntropyLoss()(logits, ya) + \
                 (1 - lam) * nn.CrossEntropyLoss()(logits, yb)
        assert not torch.isnan(loss)
