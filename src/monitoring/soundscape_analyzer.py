...
class SoundscapeAnalyzer:
    ...
    def analyze_file(
        ...
        cfg = IndicesConfig(
            sample_rate=sr, **{k: v for k, v in self.cfg.__dict__.items() if k != "sample_rate"}
        )
    ...
    def analyze_directory(
        ...
        # Encontrar archivos
        if recursive:
            files = [f for f in audio_dir.rglob("*") if f.suffix.lower() in AUDIO_EXTENSIONS]
        else:
            files = [f for f in audio_dir.glob("*") if f.suffix.lower() in AUDIO_EXTENSIONS]
        ...
        for i, path in enumerate(sorted(files)):
            ...
            try:
                result = self.analyze_file(path, windowed=windowed, window_s=window_s, hop_s=hop_s)
    ...
    def plot_indices_overview(self, df, output_dir: Path) -> Path:
        ...
        for i, idx in enumerate(indices):
            ...
            ax.violinplot(values, positions=[0], showmedians=True, showextrema=True)
            ax.scatter(
        ...
        fig.suptitle("Distribución de Índices Acústicos", fontsize=13, fontweight="bold", y=1.01)
        fig.tight_layout()
    ...
    def plot_ndsi_timeline(self, df, output_dir: Path) -> Path:
        ...
        ax.fill_between(
            x, 0, ndsi, where=ndsi >= 0, alpha=0.5, color="#4CAF50", label="Biofonia (NDSI ≥ 0)"
        )
        ax.fill_between(
            x, 0, ndsi, where=ndsi < 0, alpha=0.5, color="#F44336", label="Antrofonia (NDSI < 0)"
        )
        ax.plot(x, ndsi, color="black", lw=0.8, alpha=0.7)
        ax.set_ylim(-1.1, 1.1)
        ax.set_xlabel("Número de grabación / ventana temporal")
        ax.set_ylabel("NDSI")
        ax.set_title("Normalized Difference Soundscape Index (NDSI) — Perfil Temporal", fontsize=11)
    ...
    def plot_band_activity(self, df, output_dir: Path) -> Path:
        ...
        ax.set_title("Actividad acústica por banda de frecuencia", fontsize=11)
    ...
    def plot_site_comparison(
        ...
        for i, idx in enumerate(indices):
            ...
            ax.set_title(
                idx.upper(), fontsize=10, fontweight="bold", color=INDEX_COLORS.get(idx, "black")
            )
        ...
        fig.suptitle(
            "Comparación de Índices Acústicos entre Sitios", fontsize=13, fontweight="bold", y=1.01
        )
        fig.tight_layout()
    ...
    def plot_radar(
        ...
        if normalize:
            all_vals = {idx: [site_stats[s].get(idx, 0) for s in site_stats] for idx in indices}
    ...
    def plot_circadian(
        ...
        for ax, idx in zip(axes, indices):
            color = INDEX_COLORS.get(idx, "#607D8B")
            ax.plot(hourly.index, hourly[idx], "o-", color=color, lw=2, ms=5, label=idx.upper())
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Análisis de paisaje sonoro — Índices acústicos")
    ...
    if args.command == "file":
        ...
        if isinstance(result, list):
            ...
            for r in result[:5]:
                print(
                    f"  [{r['t_start_s']:.1f}s–{r['t_end_s']:.1f}s]"
        ...
        for idx, s in stats.items():
            print(
                f"  {idx.upper():<6} μ={s['mean']:.4f}  σ={s['std']:.4f}"
