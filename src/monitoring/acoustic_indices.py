...
class AcousticIndices:
    ...
    def compute_aci(self, Sxx: np.ndarray, j_bin: int = 5) -> float:
        """
    ...
    def compute_adi(self, freqs: np.ndarray, Sxx_db: np.ndarray) -> tuple[float, dict[str, float]]:
        """
    ...
    def compute_aei(self, freqs: np.ndarray, Sxx_db: np.ndarray) -> float:
        """
    ...
    def compute_bi(self, freqs: np.ndarray, Sxx_db: np.ndarray) -> float:
        """
        ...
        """
        mask = (freqs >= self.cfg.bi_freq_low_hz) & (freqs <= self.cfg.bi_freq_high_hz)
        if mask.sum() == 0:
            logger.warning(
                "BI: sin bins en [%.0f, %.0f] Hz", self.cfg.bi_freq_low_hz, self.cfg.bi_freq_high_hz
            )
            return 0.0
    ...
    def compute_ndsi(self, freqs: np.ndarray, Sxx: np.ndarray) -> tuple[float, float, float]:
        """
