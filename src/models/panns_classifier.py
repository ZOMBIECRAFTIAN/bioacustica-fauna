...
class CNN14Backbone(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.bn0    = nn.BatchNorm2d(64)     # normalización del input Mel
    ...
    def load_pretrained_panns(
        ...
        if not weights_path.exists():
            if not download_if_missing:
                ...
                return False
            logger.info(f"Descargando pesos PANNs-CNN14 desde Zenodo...")
        ...
        try:
            ...
            backbone_state = {
                ...
                and not k.startswith("fc1_audioset")
            }
            ...
            logger.info(f"Pesos PANNs-CNN14 cargados correctamente.")
            return True
...
class PANNsTrainer:
    ...
    def _run_epoch(
        self, optimizer, scheduler=None, train: bool = True
    ) -> Tuple[float, float]:
...
def model_comparison_table() -> str:
    ...
    rows = [
        ...
        ("PANNs-CNN14",          "~79.6M", "Log-Mel (64 bandas)", "AudioSet (2M clips)",  "Máxima transferencia"),
    ]
    sep  = "─" * 95
...
if __name__ == "__main__":
    ...
    print(f"\nPesos PANNs preentrenados: {'OK' if loaded else 'No disponibles (modo offline)'}")
