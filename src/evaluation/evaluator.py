...
class ModelEvaluator:
    ...
    def _plot_confusion_matrix(
        ...
        for ax, data, title, fmt in zip(
            ...
            sns.heatmap(
                data, annot=n <= 30, fmt=fmt, ax=ax,
                xticklabels=class_names, yticklabels=class_names,
                cmap="Blues", linewidths=0.3 if n <= 30 else 0,
            )
    ...
    def _plot_pr_curves(
        ...
        if n_classes <= 20:
            for i, (name, color) in enumerate(zip(class_names, palette)):
                ...
                a = auc(rec, prec)
                ax.plot(rec, prec, color=color, lw=1.2, alpha=0.8,
                        label=f"{name} (AP={a:.3f})")
    ...
    def _plot_calibration(
        ...
        for i in range(min(n_classes, 20)):
            ...
            ax.plot(prob_pred, prob_true, "o-", color=palette[i],
                    alpha=0.6, lw=1.2, ms=4, label=class_names[i])
        ...
        ax2.axvline(confidences.mean(), color="red", lw=2,
                    linestyle="--", label=f"Mean = {confidences.mean():.3f}")
    ...
    def _plot_per_class_bar(
        ...
        ax.axhline(np.mean(f1s), color="green", lw=1.5, linestyle="--",
...
def main():
    ...
    from src.models.cnn_baseline import load_model as load_cnn, SpectrogramDataset
    from torch.utils.data import DataLoader
    ...
    parser.add_argument("--model-type",  default="cnn_baseline",
